...
def predict_by_chunk(fe, x, chunk_size, out_arr, min_max = None):
    ...
    with stream1:
        ...
        if min_max is not None:
            ...
            min_val = cp.float32(min_val)
            max_val = cp.float32(max_val)
        
    stream1.synchronize()
    xp = cp.get_array_module(x)
    
    t1 = time.time()
    alloc_time = (t1 - t0)*1000/nb
    print("array alloc time p. input patch: %.2f ms"%alloc_time)    
    ...
    for k in range(nchunks):
        ...
        with stream1:        
            if padding != 0:
                if k == nchunks - 1:
                    x_in = xp.pad(x_in, ((0,padding), (0,0), (0,0), (0,0), (0,0)), mode = 'constant')

                if min_max is not None:
                    x_in = _rescale_data(x_in, min_val, max_val)
                
                dltensor = x_in.toDlpack()
                x_in_2 = tf.experimental.dlpack.from_dlpack(dltensor)
                x_out = fe.models['segmenter'].predict(x_in_2)
