...
def load_denoise_model(pth):
    from noise2self import unet
    import torch
    model = unet()
...
def denoise_one_frame(dn_model, frame):
    fr_tensor = torch.from_numpy(frame[np.newaxis, np.newaxis].astype(np.float32))
    dev = next(dn_model.parameters()).device
    with torch.no_grad():
        dn_img = dn_model.forward(fr_tensor.to(dev))
    return dn_img.cpu().numpy().squeeze()
