...
ply_highres = '/home/atekawade/Dropbox/Arg/transfers/runtime_plots/highres_around_void.ply'
...
if __name__ == "__main__":
    ...
    ##### BEGIN ALGORITHM ########
    # guess surface
    print(f"STEP: visualize all voids with size greater than {size_um:.2f} um")
    voids_b = Voids().guess_voids(projs, theta, center, b, b_K)    
    voids_b.select_by_size(size_um, pixel_size_um = pixel_res)

    # export all voids into a mesh
    surf = voids_b.export_void_mesh()
    ...
    print(f"STEP: visualize voids in the neighborhood of void id {void_id} at full detail")
    p_sel, r_fac = voids_b.export_grid(wd)    
    x_voids, p_voids = process_patches(projs, theta, center, fe, p_sel)
    
    # export voids to vis
    voids = Voids().import_from_grid(voids_b, x_voids, p_voids)
    surf = voids.export_void_mesh_with_texture("distance_%i"%void_id)
