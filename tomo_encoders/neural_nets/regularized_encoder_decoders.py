...
from tomo_encoders.neural_nets import change_encoders
# from tomo_encoders.neural_nets import porosity_encoders
# from tomo_encoders.neural_nets.porosity_encoders import build_CAE_3D, custom_objects_dict

from tomo_encoders.neural_nets.change_encoders import build_CAE_3D_r, custom_objects_dict
...
class SelfSupervisedCAE():
    def __init__(self, vol_shape, \
        ...
        models : dict of tf.keras.Models.model 
            dict contains {"latent_embedder" : encoder_model, "CAE" : decoder_model}. Here, both models are keras 3D models with input shape = model_size, currently CAE returns the decoder only.
...
#         loss_func = model_params["loss_func"]
        ...
        self.model_tag = "%i_%s"%(latent_dim,\
                                         descriptor_tag)

        model_keys = ["encoder", "decoder"]
    ...
    def _load_models(self, model_names = None, model_path = 'some/path'):
        ...
        ----------
        model_names : dict
            example {"encoder" : "decoder", "embedder" : "embedder_name"}
        model_path : str  
            example "some/path"
        ...
        self.model_size = self.models["decoder"].input_shape[1:-1]
        self.model_tag = "_".join(model_names["decoder"].split("_")[1:])
        return
...
class Regularized_Autoencoder(keras.Model):
    ...
    weight: float
        stregnth of the regularization loss (L1 or KL).
    ...
    def __init__(self, encoder, decoder,weight=1/250,regularization_type='kl', **kwargs):
        super(Regularized_Autoencoder, self).__init__(**kwargs)
